...
# + tags=[]
from config import PROJECT, PROJECT_PUBLIC, EEZ_INFO_TABLE, IDENTITY_TABLE, OWNER_TABLE, OWNERSHIP_BY_MMSI_TABLE, PUBLIC_FISHING_EFFORT_TABLE
...
# +
figures_folder = './figures_v20211101/'
...
#     print(q)
    ...
    job_config = bigquery.QueryJobConfig(destination=f'{PROJECT}.{OWNERSHIP_BY_MMSI_TABLE}',
...
with psm.context(psm.styles.dark):
    ...
    ax.set_title("Fishing Effort by Foreign-Owned Fishing Vessels (2012-2020)", 
                  fontsize=30, pad=15)
    plt.tight_layout()
    plt.savefig(figures_folder + 'fishing_effort_bivariate_foreign_only.png', dpi=300, bbox_inches='tight')
    plt.show()
...
with psm.context(psm.styles.dark):
    ...
    ax.set_title("Fishing Effort by Fishing Vessels with Unknown Ownership (2012-2020)", 
                  fontsize=30, pad=15)
    plt.tight_layout()
    plt.savefig(figures_folder + 'fishing_effort_bivariate_unknown_only.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def get_vessels_fishing_in_eezs(eez_list, eez_info_table, owner_table, ownership_by_mmsi_table, public_fishing_effort_table, ownership_type=None, fishing_min=24):
...
def get_vessels_fishing_in_bbox(lon_start, lat_start, lon_end, lat_end, owner_table, ownership_by_mmsi_table, ownership_type=None, fishing_min=24):
...
foreign_fishing_SPacific = get_vessels_fishing_in_bbox(124.9, -30.4, -81.2, 13.8, OWNER_TABLE, OWNERSHIP_BY_MMSI_TABLE, ownership_type='is_foreign')
...
foreign_fishing_KEN_SYC = get_vessels_fishing_in_eezs(['KEN', 'SYC'], EEZ_INFO_TABLE, OWNER_TABLE, OWNERSHIP_BY_MMSI_TABLE, f"{PROJECT_PUBLIC}.{PUBLIC_FISHING_EFFORT_TABLE}", 'is_foreign')
...
foreign_fishing_NWIO = foreign_fishing_IO = get_vessels_fishing_in_bbox(38.2, -14.8, 81.7, 12.13, OWNER_TABLE, OWNERSHIP_BY_MMSI_TABLE, ownership_type='is_foreign')
...
foreign_fishing_ARG = get_vessels_fishing_in_eezs(['ARG'], EEZ_INFO_TABLE, OWNER_TABLE, OWNERSHIP_BY_MMSI_TABLE, f"{PROJECT_PUBLIC}.{PUBLIC_FISHING_EFFORT_TABLE}", 'is_foreign')
...
foreign_fishing_FLK = get_vessels_fishing_in_eezs(['FLK'], EEZ_INFO_TABLE, OWNER_TABLE, OWNERSHIP_BY_MMSI_TABLE, f"{PROJECT_PUBLIC}.{PUBLIC_FISHING_EFFORT_TABLE}", 'is_foreign')
...
foreign_fishing_WA_EEZ = get_vessels_fishing_in_eezs(WA_eez, EEZ_INFO_TABLE, OWNER_TABLE, OWNERSHIP_BY_MMSI_TABLE, f"{PROJECT_PUBLIC}.{PUBLIC_FISHING_EFFORT_TABLE}", 'is_foreign')
...
# +
## Push the rendered notebook to the rendered/ repo
# import rendered
